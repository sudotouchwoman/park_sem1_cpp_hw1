...
timedelta_t create_timedelta(size_t years, size_t months, size_t days){
    timedelta_t new_timedelta = {
        .years = years,
        .months = months,
        .days = days
    };
...
int lies_in_offset(const char* reference, const char* date_to_check, const timedelta_t offset){
    ...
    if (sscanf(reference, "%lu-%lu-%lu", &zero_year, &zero_month, &zero_day) != 3) return 0;
    if (sscanf(date_to_check, "%lu-%lu-%lu", &target_year, &target_month, &target_day) != 3) return 0;

    const size_t days_in_year = 365;
    const size_t days_in_month = 30;

    size_t target_timedelta = (target_day - zero_day) + days_in_month * (target_month - zero_month) + days_in_year * (target_year - zero_year);
    size_t timedelta_in_days = offset.days + days_in_month * offset.months + days_in_year * offset.years;

    return target_timedelta < timedelta_in_days;
}

size_t find_reaction_of_post(const post_t* post_to_check, const timedelta_t offset){
...
size_t find_max_reaction(const blog_t* target_blog, const timedelta_t offset){
    ...
    for (size_t i = 0; i < target_blog->n_posts; ++i){
        ...
        size_t current_reaction = find_reaction_of_post(target_blog->posts[i], offset);
        if (current_reaction > max_reaction) max_reaction = current_reaction;
    }
    return max_reaction;
}

blog_t* select_most_hyped(const blog_t* target_blog, const timedelta_t offset){
    if (target_blog == NULL) return NULL;
    if (target_blog->posts == NULL) return NULL;

    size_t max_reaction = find_max_reaction(target_blog, offset);
    ...
    for (size_t i = 0; i < target_blog->n_posts; ++i){
        if (find_reaction_of_post(target_blog->posts[i], offset) < max_reaction) continue;
...
int print_most_hyped(FILE* fd_out, const blog_t* blog_to_print){
    ...
    for (size_t i = 0; i < blog_to_print->n_posts; ++i)
        print_post_contents(fd_out, blog_to_print->posts[i]);
    fprintf(fd_out, "\nProgram finished\n");
    return 0;
}
...
int print_post_contents(FILE* fd_out, const post_t* post_to_print){
    ...
    fprintf(fd_out, "\nTags: ");
    for (size_t i = 0; i < post_to_print->n_tags; ++i) 
        fprintf(fd_out, "%s ", post_to_print->tags[i]);
    fprintf(fd_out, "\nComments:\n");
